...
class AnagramChecker():
    ...
    def is_valid_word(self, word):
        if self.words.count(word.lower()) > 0:
            return True
        else:
            return False
    ...
    def get_anagrams(self, word):
        if self.is_valid_word(word):
            return [anagram for anagram in self.words if self.is_anagram(word, anagram) and word != anagram]
        else:
            return 'wrong word'


anagram_checker = AnagramChecker()
anagram_checker.get_anagrams('apple')
